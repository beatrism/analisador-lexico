...
const programa = require('./util/programa')
const dicionario = require('./util/dicionario')
...
let lexingResult = langLexer.tokenize(programa)

lexingResult.tokens.forEach(token => {
    console.log(token.image)
    console.log(token.tokenType.name)
    dicionario[token.tokenType.name] = dicionario[token.tokenType.name] + 1
})
